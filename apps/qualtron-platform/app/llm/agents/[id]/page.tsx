  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <span className="rounded-full bg-primary/15 px-2.5 py-0.5 text-xs font-medium text-primary">Enterprise</span>
        <span className="rounded-full bg-muted px-2.5 py-0.5 text-xs font-medium text-muted-foreground">hosted</span>
      </div>
      ...
      <div className="rounded-lg border border-border bg-card p-5">
        ...
        <div className="flex items-center gap-2">
          {[
            ...
            { model: '122B', label: 'Deep Analysis', tier: 'Tier 3 — Enterprise' },
          ].map((stage, i) => (
            <div key={stage.model} className="flex items-center gap-2">
              {i > 0 && <span className="text-muted-foreground">→</span>}
              <div className={`rounded-lg border p-3 text-center ${
                stage.model === '122B' ? 'border-primary/50 bg-primary/5' : 'border-border bg-muted/50'
              }`}>
                <div className="text-xs font-bold text-card-foreground">{stage.model}</div>
                <div className="mt-0.5 text-[9px] text-muted-foreground">{stage.label}</div>
                <div className="mt-0.5 text-[8px] text-muted-foreground/60">{stage.tier}</div>
              </div>
            </div>
          ))}
        </div>
        <p className="mt-3 text-xs text-muted-foreground">
          Tier 1 runs QHP non-thinking (fast retrieval from QHM). Tier 2 enables reasoning with thinking mode.
          Tier 3 (122B) produces comprehensive answers via deep analysis. QHM is pre-cached via
          RadixAttention — prefix computed once, reused across all stages.
        </p>
      </div>
      ...
      <div className="flex gap-2 border-b border-border pb-2">
        {(['overview', 'keys', 'code'] as Tab[]).map((t) => (
          <button key={t} onClick={() => setTab(t)} className={`rounded-md px-3 py-1.5 text-xs font-medium transition-colors ${
            tab === t ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted'
          }`}>
            {t === 'overview' ? 'Overview' : t === 'keys' ? 'API Keys' : 'Quick Start'}
          </button>
        ))}
      </div>
      ...
      {tab === 'overview' && (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div className="space-y-4">
            <h3 className="text-sm font-semibold">Quantum Hypergraph Memory (QHM)</h3>
            <div className="space-y-2 text-sm">
          ...
          <div className="space-y-4">
            ...
            <Link href="/llm/deploy" className="inline-flex rounded-md bg-muted px-3 py-1.5 text-xs font-medium text-muted-foreground hover:bg-muted/80">
              Deploy to GPU →
            </Link>
          </div>
          <div className="space-y-4">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm text-muted-foreground">
              API keys grant access to this Qualtron instance via the OpenAI-compatible API.
            </p>
            <button className="rounded-md bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground hover:bg-primary/90">Create Key</button>
          </div>
          <div className="rounded-lg border border-border">
            <div className="flex items-center justify-between border-b border-border px-4 py-3">
              <div>
                <span className="text-sm font-medium">Production</span>
                <span className="ml-2 font-mono text-xs text-muted-foreground">cag_prod••••••••••••</span>
              </div>
              <span className="rounded-full bg-accent/20 px-2 py-0.5 text-[10px] text-accent">Active</span>
            </div>
            <div className="flex items-center justify-between px-4 py-3">
              <div>
                <span className="text-sm font-medium">Development</span>
                <span className="ml-2 font-mono text-xs text-muted-foreground">cag_dev•••••••••••••</span>
              </div>
              <span className="rounded-full bg-accent/20 px-2 py-0.5 text-[10px] text-accent">Active</span>
            </div>
          </div>
        ...
        <div className="space-y-4">
          <div className="flex gap-2">
            {(['python', 'typescript', 'curl'] as const).map((lang) => (
              <button key={lang} onClick={() => setCodeLanguage(lang)} className={`rounded-md px-3 py-1.5 text-xs font-medium ${
                codeLanguage === lang ? 'bg-muted text-foreground' : 'text-muted-foreground hover:bg-muted/50'
              }`}>
          ...
          <p className="text-xs text-muted-foreground">
            The Qualtron API is fully OpenAI-compatible. Use any OpenAI SDK client with the Qualtron endpoint.
          </p>
        </div>
      )}
    </div>

...
const CODE_SNIPPETS = {
  python: (agentId: string) => `from openai import OpenAI
...
for chunk in response:
  ...
  typescript: (agentId: string) => `import OpenAI from "openai";
...
for await (const chunk of stream) {
  ...
  curl: (agentId: string) => `curl http://localhost:8000/v1/chat/completions \\
...
export default function LLMAgentDetailPage({
  params,
}: {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-5">
        ...
        <StatCard label="KB Tokens" value="62k" />
        <StatCard label="KB Files" value="12" />
      ...
      <div className="rounded-lg border border-border bg-card p-5">
        <h3 className="mb-3 text-sm font-semibold text-card-foreground">
          Inference Pipeline (CAG — Cache-Augmented Generation)
        </h3>
        <div className="flex items-center gap-2">
          {[
            { model: '0.8B', label: 'File Router', tier: 'Tier 1' },
            { model: '2B', label: 'Evidence Extract', tier: 'Tier 1' },
            { model: '4B', label: 'Brief Compress', tier: 'Tier 1' },
            { model: '9B', label: 'Thinker', tier: 'Tier 2' },
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div className="space-y-4">
            ...
            <div className="space-y-2 text-sm">
              <Row label="Quality" value="Maximum (122B deep thinker)" />
              <Row label="Mode" value="Hosted" />
              <Row label="Temperature" value="0.7" />
              <Row label="Max Tokens" value="2,048" />
              <Row label="Model ID" value={id} mono />
            </div>
          </div>
          <div className="space-y-4">
            <h3 className="text-sm font-semibold">Knowledge Base</h3>
            <div className="space-y-2 text-sm">
              <Row label="Token Count" value="62,000" />
              <Row label="File Count" value="12" />
              <Row label="Strategy" value="single_pass" mono />
              <Row label="Cache Status" value="Warmed" />
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm text-muted-foreground">
              API keys grant access to this agent via the OpenAI-compatible API.
            </p>
      ...
      {tab === 'code' && (
        <div className="space-y-4">
          ...
          <p className="text-xs text-muted-foreground">
            The API is fully OpenAI-compatible. Replace the base URL and API key
            to use with any OpenAI SDK client.
          </p>
        </div>
      )}
    </div>

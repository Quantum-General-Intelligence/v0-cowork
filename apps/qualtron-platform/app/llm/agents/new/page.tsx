...
export default function CreateCAGModelPage() {
  ...
  const handleCreateAgent = async (e: FormEvent) => {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok)
        throw new Error(data.detail?.[0]?.msg ?? data.error ?? 'Failed')
      setAgentId(data.id)
      setStep('upload')
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <div className="flex gap-1">
        {['Model', 'Config', 'Upload QHM', 'Processing', 'Deploy', 'Ready'].map(
          (label, i) => (
            <div key={label} className="flex items-center gap-1">
              {i > 0 && (
                <div
                  className={`h-px w-4 ${stepIndex >= i ? 'bg-primary' : 'bg-border'}`}
                />
              )}
              <div
              ...
              <span
                className={`text-[10px] ${stepIndex >= i ? 'text-foreground' : 'text-muted-foreground'}`}
              >
                {label}
              </span>
            </div>
      ...
      {step === 'model' && (
        <div className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Select a Qualtron model variant from the catalog:
          </p>
            ...
            {catalog.map((v) => (
              ...
              >
                ...
                <div className="mt-1 text-xs text-muted-foreground">
                  {(v.context_tokens / 1000).toFixed(0)}K ctx · {v.vram_gb}GB
                  VRAM · {v.avg_latency_s}s latency
                </div>
              </button>
            ))}
          </div>
        ...
        <form onSubmit={handleCreateAgent} className="space-y-4">
          <div className="rounded-lg border border-primary/30 bg-primary/5 p-3">
            ...
            <span className="ml-2 text-xs text-muted-foreground">
              {selectedModel &&
                `${(selectedModel.context_tokens / 1000).toFixed(0)}K context`}
            </span>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="mb-3 text-xs text-muted-foreground">
              ...
              is configured automatically based on your selection.
            </p>
      ...
      {step === 'upload' && (
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Upload documents to build the QHM (Quantum Hypergraph Memory) for{' '}
            <strong>{agentName}</strong>. Files are processed through the CAG
            ingest pipeline.
          </p>
          <div
          ...
          >
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              PDF, DOCX, TXT, MD, CSV, JSON, code — up to 50MB
            </p>
            <input
              ref={fileInputRef}
              type="file"
        ...
        <div className="space-y-4">
          <div className="rounded-lg border border-accent/50 bg-accent/10 p-4">
            <p className="text-sm font-medium text-accent">
              QHM ready for {agentName}
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
              Now deploy the model to GPU to start serving.
            </p>
          </div>
          <button
